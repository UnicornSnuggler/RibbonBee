    ...
    async execute(interaction) {        
        try {
            ...
            if (!commandOption) {
                ...
                description.push(`5. "Galar", "Sinnoh", "Hisui", and "Paldea" represent the specified Pokémon's presence in all four of the generation 8/9 Switch games. A ${AFFIRMATIVE_EMOJI} indicates that the specified Pokémon is present in that game's regional Pokédex and can collect its ribbons. A ${NEGATIVE_EMOJI} indicates that the specified Pokémon was cut from the game's regional Pokédex and cannot collect its ribbons.`);
                description.push(`6. The "Ribbon Totals" section outlines a close approximation of the number of ribbons that the specified Pokémon is able to acquire throughout its Ribbon Mastery journey starting from the indicated generation.\n* "Guaranteed" represents ribbons that the specified Pokémon will always be able to obtain under the given circumstances.\n* "Possible" represents ribbons that may or may not be obtainable for the specified Pokémon based on any number of circumstances.\n* "Contingent" represents conflicting ribbons that the specified Pokémon cannot simultaneously earn, necessitating a choice to be made between the set.`);
                ...
                description.push(`* \`verbose\` — Used to retrieve a raw list of ribbon names that the specified Pokémon is able to acquire based on the queried parameters. **Note**: If this command is used outside of direct messages or the designated RibbonBee channel, it will render as Ephemeral — which means no other users will be able to see its output. *(This is to avoid flooding channels with walls of text.)*`);
            }
